...
public class MinHeap<E, P extends Comparable<P>> implements MinPriorityQueue<E,P> {
	...
	private void swap(int i, int j) {
		...
		HeapPQEntry a = entries.get(i);
		HeapPQEntry b = entries.get(j);
		a.position = j ;
		b.position = i;
		entries.set(i, b);
		entries.set(j, a);
	...
	private void repairHeapAtEntry(MinHeap<E, P>.HeapPQEntry entry) {
		// TODO 
			bubbleUp(entries.indexOf(entry));
			bubbleDown(entries.indexOf(entry));
			ticker.tick();
	}
	...
	public PQEntry<E, P> insert(E thing, P priority) {
		// TODO
		MinHeap<E, P>.HeapPQEntry entry = new HeapPQEntry(thing , priority);
		entry.position = entries.size();
		entries.add(entry);
		entries.getLast().position = entries.size() - 1;
		repairHeapAtEntry(entry);
		ticker.tick();
		return entry;
	}
	...
	public PQEntry<E, P> extractMin() {
		...
		entries.removeLast();
		repairHeapAtEntry(entries.getFirst());
        return entry;
	}
	...
	private void bubbleDown(int startIndex) {
		// TODO
		ticker.tick();
		if(leftChildIndex(startIndex) >= entries.size() && rightChildIndex(startIndex) >= entries.size()) return;
		if(rightChildIndex(startIndex) >= entries.size()) {
			if(entries.get(startIndex).getPriority().compareTo(entries.get(leftChildIndex(startIndex)).getPriority()) > 0) {
				swap(startIndex , leftChildIndex(startIndex));
				bubbleDown(leftChildIndex(startIndex));
			}
			return;
		}
		else {
			P sp = entries.get(startIndex).getPriority();
			P lp = entries.get(leftChildIndex(startIndex)).getPriority();
			P rp = entries.get(rightChildIndex(startIndex)).getPriority();
			if(sp.compareTo(lp) > 0 || sp.compareTo(rp) > 0) {
				if(lp.compareTo(rp) < 0) {
					swap(startIndex , leftChildIndex(startIndex));
					bubbleDown(leftChildIndex(startIndex));
				}
				if(lp.compareTo(rp) > 0) {  
					swap(startIndex , rightChildIndex(startIndex));
					bubbleDown(rightChildIndex(startIndex));
				}
			}
			return;
		}
	...
	private void bubbleUp(int startIndex) {
		...
		P sp = entries.get(startIndex).getPriority();
		P pp = entries.get(parentIndex(startIndex)).getPriority();
		if(sp.compareTo(pp) < 0) {
			swap(startIndex , parentIndex(startIndex));
			bubbleUp(parentIndex(startIndex));
		}
		return;
	}
